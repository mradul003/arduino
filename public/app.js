...
async function flashHex(port, hexText, onProgress) {
  ...
  try {
    ...
    link.clear();
    await link.write([STK_ENTER_PROGMODE, CRC_EOP]);
    await expectInsyncOk(link, 1000);
    ...
    for (let page = 0; page < totalPages; page++) {
      ...
      link.clear();
      await link.write([STK_LOAD_ADDRESS, wordAddr & 0xff, (wordAddr >> 8) & 0xff, CRC_EOP]);
      await expectInsyncOk(link, 800);
      ...
      await expectInsyncOk(link, 1500);

      onProgress((page + 1) / totalPages);
    }

    link.clear();
    await link.write([STK_LEAVE_PROGMODE, CRC_EOP]);
    await expectInsyncOk(link, 1000);
